    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success) {
                ...
                toastr.success(response.message, 'Create Company');
            }
            else {
                if (response.errors != null) {
                    global.displayValidationErrors(response.errors);
                } else {
                    toastr.error(response.message, 'Create Company', { closeButton: true, timeOut: 0, extendedTimeOut: 0 });
                }
            }
...
function UploadCompanyLogo(strName) {
    ...
    $.ajax({
        ...
        error: function (xhr, txtStatus, errThrown) {
            toastr.error('Error: ' + xhr.statusText, 'Upload Company Logo', { closeButton: true, timeOut: 0, extendedTimeOut: 0 });
        }
    });
};
